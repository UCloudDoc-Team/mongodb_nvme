...
## 如何预估MongoDB从备份恢复至新实例用时?

当使用MongoDB从备份恢复至新实例功能时，恢复时间取决于备份数据量和目标集群规格。备份数据量越小，目标集群规格越高, 恢复完成用时越短。
在逻辑备份方式下, 一般数据恢复速度大概在10~20G/小时。
案例一: 副本集, 备份数据量大小42G, 备份文件大小11G, 目标集群计算规格4C8G, 磁盘容量100G, 恢复完成耗时3小时48分钟。
案例二: 副本集, 备份数据量大小49G, 备份文件大小13G, 目标集群计算规格8C16G, 磁盘容量240G, 恢复完成耗时2小时27分钟。
